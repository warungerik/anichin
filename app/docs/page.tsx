    ...
    <>
      <style>{`
        :root {
          ...
          --get: #6af7c4; --get-bg: rgba(106,247,196,0.08); --code-bg: #0d0d16;
        }
        * { box-sizing: border-box; margin: 0; padding: 0; }
        html { scroll-behavior: smooth; }
        body { background: var(--bg); color: var(--text); font-family: 'Syne', sans-serif; line-height: 1.6; min-height: 100vh; }
        ...
        .nav-item.status-link { color: var(--accent2); border-color: rgba(247,106,140,0.2); background: rgba(247,106,140,0.05); }
        .nav-item.status-link:hover { color: var(--accent2); border-color: rgba(247,106,140,0.4); background: rgba(247,106,140,0.1); }
        ...
        .status-banner { display: flex; align-items: center; justify-content: space-between; gap: 1rem; background: rgba(247,106,140,0.06); border: 1px solid rgba(247,106,140,0.2); border-radius: 10px; padding: 1rem 1.5rem; margin-bottom: 2rem; }
        .status-banner-text { font-family: 'JetBrains Mono', monospace; font-size: 0.8rem; color: var(--text); }
        .status-banner-text span { color: var(--accent2); font-weight: 700; }
        .status-banner-btn { background: var(--accent2); color: white; border: none; border-radius: 6px; padding: 0.5rem 1rem; font-family: 'Syne', sans-serif; font-size: 0.8rem; font-weight: 700; cursor: pointer; text-decoration: none; white-space: nowrap; display: inline-block; transition: opacity 0.15s; }
        .status-banner-btn:hover { opacity: 0.85; }
        ...
        .try-btn:hover { opacity: 0.85; } .try-btn:disabled { opacity: 0.5; cursor: not-allowed; }
        ...
        <aside className="sidebar">
          ...
          <div className="nav-section">
            ...
            {['home', 'search', 'series', 'watch', 'schedule', 'ongoing', 'completed', 'sidebar', 'genres', 'quickfilter'].map(ep => (
              <a key={ep} href={`#ep-${ep}`} className="nav-item">
          ...
          <div className="nav-section">
            ...
            <a href="/status" className="nav-item status-link">
              <span className="method-dot" style={{ background: 'var(--accent2)', animation: 'pulse 2s infinite' }}></span>
              API Status Checker
            </a>
          </div>
        ...
        <main className="main">
          ...
          <div className="status-banner">
            <div className="status-banner-text">
              🔍 Mau cek apakah <span>API kamu berjalan</span> dengan benar?
            </div>
            <a href="/status" className="status-banner-btn">▶ Run API Tests</a>
          </div>
          ...
          <div>
            ...
            <div className="set-url-row">
              ...
              <button className="set-btn" onClick={() => {
                const el = document.getElementById('baseUrlInput') as HTMLInputElement;
                const val = el?.value?.trim().replace(/\/$/, '');
                if (val) { (window as any).__BASE__ = val; const d = document.getElementById('baseUrlDisplay'); if (d) d.textContent = val; }
              }}>Set URL</button>
            </div>
          </div>
            ...
            <table className="overview-table">
              ...
              <tbody>
                {[
                  ...
                  <tr key={path}><td className="ov-method">GET</td><td className="ov-path">{path}</td><td className="ov-desc">{desc}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
  ...
  "message": null       // string  — pesan error jika gagal
          ...
          <div className="section" id="examples">
            ...
            <div className="code-block" style={{ marginBottom: '1.25rem' }}>
              <div className="code-header"><span className="code-lang">JavaScript / Fetch</span><button className="copy-btn card-copy-btn">copy</button></div>
              <div className="code-content">{`const BASE = "https://anichin-lovat.vercel.app";
...
console.log(ep.data.watch.servers);`}</div>
            ...
            <div className="code-block" style={{ marginBottom: '1.25rem' }}>
              <div className="code-header"><span className="code-lang">Python / requests</span><button className="copy-btn card-copy-btn">copy</button></div>
              <div className="code-content">{`import requests

BASE = "https://anichin-lovat.vercel.app"

res = requests.get(f"{BASE}/api/search", params={"q": "naruto", "page": 1})
print(res.json()["data"]["search"]["lists"])

res = requests.get(f"{BASE}/api/series/against-the-sky-supreme")
print(res.json()["data"]["detail"]["title"])`}</div>
            </div>

            <div className="code-block">
              <div className="code-header"><span className="code-lang">PHP</span><button className="copy-btn card-copy-btn">copy</button></div>
              <div className="code-content">{`<?php
$base = "https://anichin-lovat.vercel.app";
$data = json_decode(file_get_contents("$base/api/search?q=naruto"), true);
print_r($data['data']['search']['lists']);`}</div>
            </div>
          </div>
      ...
      <script dangerouslySetInnerHTML={{
        __html: `
        window.__BASE__ = '';
        document.querySelectorAll('.copy-btn, .card-copy-btn').forEach(btn => {
          btn.addEventListener('click', function(e) {
            e.stopPropagation();
            const el = this.closest('.code-block')?.querySelector('.code-content');
            if (!el) return;
        ...
        document.querySelectorAll('.endpoint-header').forEach(h => {
          h.addEventListener('click', () => h.closest('.endpoint-card').classList.toggle('open'));
        });
        document.querySelectorAll('.try-btn').forEach(btn => {
          btn.addEventListener('click', async function(e) {
            e.stopPropagation();
            const input = this.closest('.try-it-row').querySelector('.try-url');
...
function EndpointCard({ method, path, desc, id, description, pathParams, queryParams, curlExample, defaultPath }: {
  ...
  return (
    <div className="endpoint-card" id={id}>
      ...
      <div className="endpoint-body">
        {description && <div className="endpoint-section"><div className="endpoint-section-title">Description</div><p style={{ fontSize: '0.85rem', color: 'var(--muted)' }}>{description}</p></div>}
        {pathParams?.length ? <div className="endpoint-section"><div className="endpoint-section-title">Path Parameters</div><table className="params-table"><thead><tr><th>Name</th><th>Type</th><th>Required</th><th>Description</th></tr></thead><tbody>{pathParams.map(p => <tr key={p.name}><td className="param-name">{p.name}</td><td className="param-type">{p.type}</td><td className={p.req ? 'param-required' : 'param-optional'}>{p.req ? 'required' : 'optional'}</td><td className="param-desc">{p.desc}</td></tr>)}</tbody></table></div> : null}
        {queryParams?.length ? <div className="endpoint-section"><div className="endpoint-section-title">Query Parameters</div><table className="params-table"><thead><tr><th>Name</th><th>Type</th><th>Required</th><th>Description</th></tr></thead><tbody>{queryParams.map(p => <tr key={p.name}><td className="param-name">{p.name}</td><td className="param-type">{p.type}</td><td className={p.req ? 'param-required' : 'param-optional'}>{p.req ? 'required' : 'optional'}</td><td className="param-desc">{p.desc}</td></tr>)}</tbody></table></div> : null}
        <div className="endpoint-section"><div className="endpoint-section-title">Example Request</div><div className="code-block"><div className="code-header"><span className="code-lang">curl</span><button className="copy-btn card-copy-btn">copy</button></div><div className="code-content">{curlExample}</div></div></div>
        <div className="try-it"><div className="endpoint-section-title" style={{ marginBottom: '0.75rem' }}>Try It</div><div className="try-it-row"><input className="try-url" type="text" defaultValue={defaultPath} /><button className="try-btn">Send →</button></div><div className="try-result"></div></div>
      </div>
    </div>
  );
}
